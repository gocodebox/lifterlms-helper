...
<form action="" class="llms-beta-main" method="POST">
	...
	<aside class="llms-beta-aside">
		...
		<p><strong><?php _e( 'Beta releases may not be stable. We may not be able to fix issues caused by using a beta release. We urge you to only use beta versions in testing environments!', 'lifterlms' ); ?></strong></p>
		...
		<p><strong><?php _e( 'This plugin does not provide you with the ability to rollback from a beta version.', 'lifterlms' ); ?></strong></p>
		<p><?php _e( 'To rollback you should subscribe to the stable channel, delete the beta version of the plugin, and then re-install the latest version. If a database migration was run you should also restore your database from a backup.', 'lifterlms' ); ?></p>

		<h3><?php _e( 'Reporting bugs and contributing', 'lifterlms' ); ?></h3>
		<p>
			<?php
				// Translators: %1$s = Opening anchor link; %2$s = closing anchor link.
				printf( __( 'We welcome contributions of all kinds, review our contribution guidelines on %1$sGitHub%2$s to get started.', 'lifterlms' ), '<a href="https://github.com/gocodebox/lifterlms/blob/master/.github/CONTRIBUTING.md">', '</a>' );
			?>
		</p>
		<p>
			<?php
				// Translators: %s = Link to bug report.
				printf( __( 'If you encounter a bug while beta testing, please report it at %s.', 'lifterlms' ), make_clickable( 'https://github.com/gocodebox/lifterlms/issues' ) );
			?>
		</p>
		...
		<p>
			<?php
				...
				printf( __( 'Check out our Guide to Beta Testing at %s.', 'lifterlms' ), make_clickable( 'https://lifterlms.com/docs/beta-testing/' ) );
			?>
		</p>
